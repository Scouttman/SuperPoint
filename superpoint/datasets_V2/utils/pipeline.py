...
from superpoint.datasets_V2.utils import photometric_augmentation as photaug
from superpoint.models_V2.homographies import (sample_homography, compute_valid_mask,
...
def add_keypoint_map(data):
    with tf.compat.v1.name_scope('add_keypoint_map'):
        ...
        points = points[~tf.math.is_nan(points[:,0])] # remove nans (long story)
        kp = tf.minimum(tf.cast(tf.round(points), dtype=tf.int32), image_shape-1)
        kmap = tf.scatter_nd(
