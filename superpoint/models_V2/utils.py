...
def detector_head(inputs, **config):
    ...
    with tf.compat.v1.variable_scope('detector', reuse=tf.compat.v1.AUTO_REUSE):
        ...
        prob = tf.compat.v1.depth_to_space(
...
def descriptor_head(inputs, **config):
    ...
    with tf.compat.v1.variable_scope('descriptor', reuse=tf.compat.v1.AUTO_REUSE):
        ...
        desc = tf.transpose(a=x, perm=[0, 2, 3, 1]) if cfirst else x
        desc = tf.image.resize(
            desc, config['grid_size'] * tf.shape(input=desc)[1:3], method=tf.image.ResizeMethod.BILINEAR)
        desc = tf.transpose(a=desc, perm=[0, 3, 1, 2]) if cfirst else desc
...
def detector_loss(keypoint_map, logits, valid_mask=None, **config):
    ...
    labels = tf.cast(keypoint_map[..., tf.newaxis], dtype=tf.float32)  # for GPU
    labels = tf.compat.v1.space_to_depth(input=labels, block_size=config['grid_size'])
    shape = tf.concat([tf.shape(input=labels)[:3], [1]], axis=0)
    labels = tf.concat([2*labels, tf.ones(shape)], 3)
    # Add a small random matrix to randomly break ties in argmax
    labels = tf.argmax(input=labels + tf.random.uniform(tf.shape(input=labels), 0, 0.1),
                       axis=3)

    # Mask the pixels if bordering artifacts appear
    valid_mask = tf.ones_like(keypoint_map) if valid_mask is None else valid_mask
    valid_mask = tf.cast(valid_mask[..., tf.newaxis], dtype=tf.float32)  # for GPU
    valid_mask = tf.compat.v1.space_to_depth(input=valid_mask, block_size=config['grid_size'])
    valid_mask = tf.reduce_prod(input_tensor=valid_mask, axis=3)  # AND along the channel dim
...
def descriptor_loss(descriptors, warped_descriptors, homographies,
    ...
    (batch_size, Hc, Wc) = tf.unstack(tf.cast(tf.shape(input=descriptors)[:3], dtype=tf.int32))
    ...
    cell_distances = tf.norm(tensor=coord_cells - warped_coord_cells, axis=-1)
    s = tf.cast(tf.less_equal(cell_distances, config['grid_size'] - 0.5), dtype=tf.float32)
    ...
    dot_product_desc = tf.reduce_sum(input_tensor=descriptors * warped_descriptors, axis=-1)
    ...
    normalization = tf.reduce_sum(input_tensor=valid_mask) * tf.cast(Hc * Wc, dtype=tf.float32)
    ...
    tf.compat.v1.summary.scalar('positive_dist', tf.reduce_sum(input_tensor=valid_mask * config['lambda_d'] *
                                                     s * positive_dist) / normalization)
    tf.compat.v1.summary.scalar('negative_dist', tf.reduce_sum(input_tensor=valid_mask * (1 - s) *
                                                     negative_dist) / normalization)
    loss = tf.reduce_sum(input_tensor=valid_mask * loss) / normalization
    return loss
...
def spatial_nms(prob, size):
    ...
    with tf.compat.v1.name_scope('spatial_nms'):
        # prob = tf.expand_dims(tf.expand_dims(prob, axis=0), axis=-1) 
        prob = tf.expand_dims(prob, axis=-1)
        pooled = tf.nn.max_pool2d(
                input=prob, ksize=[1, size, size, 1], strides=[1, 1, 1, 1], padding='SAME')
        prob = tf.compat.v1.where(tf.equal(prob, pooled), prob, tf.zeros_like(prob))
...
def box_nms(prob, size, iou=0.1, min_prob=0.01, keep_top_k=0):
    ...
    """
    # with tf.compat.v1.name_scope('box_nms'):
    with tf.name_scope('box_nms'):
        pts = tf.cast(tf.compat.v1.where(tf.greater_equal(prob, min_prob)), dtype=tf.float32)
        pts = pts[:,1:]
        ...
        with tf.device('/cpu:0'):
            indices = tf.image.non_max_suppression(
                    boxes, scores, tf.shape(input=boxes)[0], iou)
        pts = tf.gather(pts, indices)
        scores = tf.gather(scores, indices)
        if keep_top_k:
            k = tf.minimum(tf.shape(input=scores)[0], tf.constant(keep_top_k))  # when fewer
            scores, indices = tf.nn.top_k(scores, k)
            pts = tf.gather(pts, indices)
        prob = tf.scatter_nd(tf.cast(pts, dtype=tf.int32), scores, tf.shape(input=prob))
    return prob
